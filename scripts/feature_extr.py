...
class FeatureExtractionLayer:
    @staticmethod
    def train_add_months_since_last_sale(
        aggregated_df,
        load_precalculated=False,
        precalculated_path="../data/aggregated_months.csv",
    ):

        if load_precalculated:
            aggregated_sorted = pd.read_csv(precalculated_path)
        else:
            aggregated_sorted = aggregated_df.sort_values(
                by=["item_id", "date_block_num"]
        ...
        aggregated_train_df["months_since_last_sale"] = aggregated_train_df[
    ...
    @staticmethod
    def test_add_months_since_last_sale(
        aggregated_df,
        test_df,
        load_precalculated=False,
        precalculated_path="../data/test_months.csv",
    ):

        if load_precalculated:
            test_df = pd.read_csv(precalculated_path)
        else:
            train_subdf = aggregated_df.copy()[
                ["date_block_num", "item_id", "months_since_last_sale"]
            ]
            ...
            united_subdf_sorted = united_subdf.sort_values(
                by=["item_id", "date_block_num"]
            ).reset_index(drop=True)
            ...
            test_df = test_df.merge(
                united_subdf_sorted, on=["item_id", "is_test"], how="left"
            )

            test_df.drop(columns=["is_test", "date_block_num"], inplace=True)
            test_df = test_df.loc[
                test_df.groupby("ID")["months_since_last_sale"].idxmax()
            ]  # drop duplicates along the "ID" feature
            test_df.to_csv("../data/test_months.csv", index=False)

        test_df = transform_df_types(test_df)

        test_df["months_since_last_sale"] = test_df["months_since_last_sale"].replace(
            -1, 0
        )
    ...
    def calculate_historical_mean(
        df, column_name="item_cnt_month", on_columns=["item_id", "shop_id"]
    ):
        on_columns_name = "_".join(on_columns)
        df = df.sort_values(by=on_columns + ["date_block_num"])
        df_aggregated = (
            df.groupby(on_columns + ["date_block_num"])[column_name]
            .mean()
            .reset_index()
        )
        df_aggregated[column_name + "_mean_on_" + on_columns_name] = (
            df_aggregated.groupby(on_columns)[column_name]
            .apply(lambda x: x.shift().expanding().mean())
            .reset_index(name="historical_mean")["historical_mean"]
        )
    ...
    @staticmethod
    def test_calculate_historical_mean(
        test, df, column_name="item_cnt_month", on_columns=["item_id", "shop_id"]
    ):
        on_columns_name = "_".join(on_columns)
        df_aggregated = (
            df.groupby(on_columns)[column_name]
            .mean()
            .reset_index(name=column_name + "_mean_on_" + on_columns_name)
        )
        test = test.merge(df_aggregated, on=on_columns, how="left")
        return test

    @staticmethod
    def train_add_lag_features(
        df,
        col,
        add_name="",
        ...
        for lag in lags:
            df["{}_lag_{}".format(col + add_name, lag)] = df[
                "{}_lag_{}".format(col + add_name, lag)
            ].fillna(0)
        return df
    ...
    def test_add_lag_features(
        ...
        col,
        add_name="",
        on_columns=["shop_id", "item_id"],
        operation="mean",
        lags=[1, 2, 3],
    ):
        new_df = original_df[on_columns + ["date_block_num", col]]
        new_df = (
            new_df.groupby(on_columns + ["date_block_num"])
            .agg({col: operation})
            .reset_index()
        )
        for lag in lags:
    ...
    @staticmethod
    def test_add_lag_features_for_month(
        df,
        original_df,
        col,
        add_name="",
    ...
    @staticmethod
    def train_transform(df, aggregated_df, clip_target=True, for_airflow=False):
        ...
        train_aggregated = (
            df.groupby(["date_block_num", "shop_id"])
            .agg({"revenue": "sum"})
            .reset_index()
        )
        aggregated_df = aggregated_df.merge(
            train_aggregated, on=["date_block_num", "shop_id"], how="left"
        )
        aggregated_df["revenue"].fillna(0, inplace=True)

        # 3. Calculate historical mean revenue information
        aggregated_df = FeatureExtractionLayer.calculate_historical_mean(
            aggregated_df, "revenue", ["shop_id"]
        )
        ...
        aggregated_lagged["item_cnt_month"] = aggregated_lagged["item_cnt_month"].clip(
            lower=0
        )
        if clip_target:
            aggregated_lagged["item_cnt_month"] = aggregated_lagged[
                "item_cnt_month"
            ].clip(upper=20)

        # 6. Calculate historical mean item_cnt_month information
        aggregated_lagged = FeatureExtractionLayer.calculate_historical_mean(
            aggregated_lagged, "item_cnt_month", on_columns=["item_id"]
        )
        aggregated_lagged = FeatureExtractionLayer.calculate_historical_mean(
            aggregated_lagged, "item_cnt_month", on_columns=["item_category_id"]
        )
        aggregated_lagged["item_cnt_month_mean_on_item_id"].fillna(
            aggregated_lagged["item_cnt_month_mean_on_item_category_id"], inplace=True
        )
        aggregated_lagged.drop(
            "item_cnt_month_mean_on_item_category_id", axis=1, inplace=True
        )
        ...
        aggregated_lagged = FeatureExtractionLayer.train_add_lag_features(
            ...
            "item_cnt_month",
            add_name="_cat_",
            on_columns=["item_category_id", "date_block_num"],
            lags=[1, 2, 3],
        )
        ...
        aggregated_lagged = FeatureExtractionLayer.train_add_lag_features(
            ...
            "item_cnt_month",
            add_name="_item_",
            on_columns=["date_block_num", "item_id"],
            lags=[1, 2, 3],
        )
        ...
        aggregated_lagged = FeatureExtractionLayer.train_add_lag_features(
            ...
            "item_cnt_month",
            add_name="_cat_shop_",
            on_columns=["item_category_id", "shop_id", "date_block_num"],
            lags=[1, 2, 3],
        )
        ...
        avg_item_price = (
            aggregated_lagged.groupby(["item_id", "date_block_num"])["item_price"]
            .mean()
            .reset_index()
        )
        avg_item_price.columns = ["item_id", "date_block_num", "avg_item_price"]
        aggregated_lagged = aggregated_lagged.merge(
    ...
    def test_transform(df, aggregated_df, test_df, clip_target=True):
        ...
        test_df = FeatureExtractionLayer.test_calculate_historical_mean(
            test_df, aggregated_df, "revenue", ["shop_id"]
        )

        # 4. Add revenue lagged features
        test_lagged = FeatureExtractionLayer.test_add_lag_features(
            test_df,
            aggregated_df,
            col="revenue",
            on_columns=["shop_id"],
        ...
        if clip_target:
            aggregated_df["item_cnt_month"] = aggregated_df["item_cnt_month"].clip(
                upper=20
            )

        # 5. Calculate historical mean "item_cnt_month"
        test_lagged = FeatureExtractionLayer.test_calculate_historical_mean(
            test_lagged, aggregated_df, "item_cnt_month", on_columns=["item_id"]
        )
        test_lagged = FeatureExtractionLayer.test_calculate_historical_mean(
            test_lagged,
            aggregated_df,
            "item_cnt_month",
            on_columns=["item_category_id"],
        )
        test_lagged["item_cnt_month_mean_on_item_id"].fillna(
            test_lagged["item_cnt_month_mean_on_item_category_id"], inplace=True
        )
        test_lagged.drop(
            "item_cnt_month_mean_on_item_category_id", axis=1, inplace=True
        )
        ...
        test_lagged = FeatureExtractionLayer.test_add_lag_features(
            ...
            aggregated_df,
            col="item_cnt_month",
            on_columns=["item_id", "shop_id"],
            lags=[1, 2, 3],
        )

        # 7. Add average "item_cnt_month" lagged features by month
        test_lagged["date_block_num"] = 34
        test_lagged = FeatureExtractionLayer.test_add_lag_features_for_month(
            test_lagged,
            aggregated_df,
            col="item_cnt_month",
            add_name="_date_",
        ...
        test_lagged = FeatureExtractionLayer.test_add_lag_features(
            ...
            col="item_cnt_month",
            add_name="_cat_shop_",
            on_columns=["item_category_id", "shop_id"],
            lags=[1, 2, 3],
        )
        ...
        # 13. Add "item_price" lag features
        avg_item_price = (
            aggregated_df.groupby(["item_id", "date_block_num"])["item_price"]
            .mean()
            .reset_index()
        )
        avg_item_price.columns = ["item_id", "date_block_num", "avg_item_price"]
